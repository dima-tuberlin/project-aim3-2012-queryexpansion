...
public class SessionReducer
  ...
  private static final List<String> FILTER_QUERY = Arrays.asList();
  ...
  public void reduce(Iterator<PactRecord> records, Collector<PactRecord> out)
    ...
    PriorityQueue<PactRecord> logEntries = new PriorityQueue<PactRecord>(10, new Comparator<PactRecord>() {
    ...
    while (logEntries.size() != 0) {
      ...
      // System.out.println("from que: " + time + " " + query + " " + doc + "line: "
      // + logEntry.getField(10, PactString.class));
      ...
      if (!(query.equals(lastQuery) && doc.equals(lastUrl))) {
        ...
        Iterator<String> sessionIter = activeSessionsMap.keySet().iterator();
        Session session;
        String sessionQuery = "";
        ...
        while (sessionIter.hasNext()) {
          ...
          if (TimeUtils.timeDiffInSec(session.getTime(), time) <= MAX_SEC_INTERVAL) {
            ...
            // System.out.println("Time emit q: " + session.getQuery());
        ...
        if (!query.equals(lastQuery)) {
          ...
          // add new active session
          // TODO re-think: only add if no active session for this query
          // exists
          // if (query.equals("mars") || query.equals("jaguar") || query.equals("internet") ||
          // query.equals("irak")) {
          if (FILTER_QUERY.contains(query)) {
            if (!activeSessionsMap.containsKey(query)) {
              activeSessionsMap.put(query, new Session(query, time));
            }
          }
        }
      } else {
   ...
   * 
   * A valid session contains at least one refinement with a doc or two refinements.
   * 
   * @param session
   * @param out
   */
  private void emitSession(String userId, Session session, Collector<PactRecord> out) {
    ...
    if (sessionRefineCount > 0) {
      ...
      if (!(sessionRefineCount == 1 && session.getDocs().get(0).equals(EMPTY_QUERY))) {
        validSession = true;
      }
    }
    ...
    if (validSession) {
      ...
      // TODO don't double emit equal entries without doc, because co occ is only once per session
      ...
      for (int i = 0; i < sessionRefineCount; i++) {
        ...
        // System.out.println("Ses: " + sessionId.getValue() + " " + query.getValue() + "\t|\t" +
        // ref.getValue() + " "
        // + doc.getValue());
